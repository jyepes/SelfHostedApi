...
namespace Client
{
    class Program
    {
        ...
        static void ListAllItems()
        {
            ...
            var items = resp.Content.ReadAsAsync<IEnumerable<Item>>().Result;
        ...
        {
            var resp = client.GetAsync(string.Format("api/items/{0}", id)).Result;
            //resp.EnsureSuccessStatusCode();  
            var item = resp.Content.ReadAsAsync<Item>().Result;
